...
* 功能: 服务治理理、虚拟化、水平扩容、问题定位、性能压测、系统监控、兼容遗留系统......
...
基于 SPI 设计思路路，提供基础服务的⼯工⼚厂，动态实例例化对应 interface
...
```
HTTP 协议:包装 http.Handler，⽤用责任链模式处理理 http.Request 和 http.ResponseWriter
RPC 协议:劫持协议序列列化流程，⽤ FSM(有限状态机)来跟踪序列列化过程并适时修改数据
...
可透明的在服务间传递上下⽂文信息，从⽽实现流量染色、调用跟踪、防雪崩等功能
```
...
[低精度 timer 实现原理理](https://blog.acolyer.org/2015/11/23/hashed-and-hierarchical-timing-wheels/)
...
* 可⽀撑百万级⽇订单规模、万级并发长连接
* 业务收益
* 零成本: 透明接⼊入公司运维、发布、⽇日志、压测等平台，⽀支持服务注册发现、弹性伸缩等能⼒力力
